...
| 服务器名称         | 功能                                                         |
| ------------------ | ------------------------------------------------------------ |
| ct_db_proxy_server | 数据库DB的相关CAUD操作                                       |
| ct_file_server     | 负责文件相关的传输，包括在线传输和离线传输                   |
| ct_http_msg_server | 负责WebSocket消息传输，和消息相关的解析操作（对外消息接口，提供对外接口功能） |
| ct_msfs            | 图片存储服务器，提供头像，图片传输的图片存储服务             |
| ct_msg_server      | 主消息服务器，包含一些登录消息处理实例（负载较小）和一些其他消息实例 |
| ct_push_server     | 推送服务器，负责消息的推送服务，提供IOS消息的推送            |
| ct_route_server    | 路由服务器，负责消息的转发                                   |

### 网络相关处理

```c++
1.netlib封装了响应的Socket操作,通过普通函数调用全局单例对象来进行处理
2.BaseSocket封装了网络库的相关处理,包括端口绑定,监听,连接等操作,也实现了读,写,异常事件处理
3.EventDispatch封装了相关事件处理,包括定时任务和循环任务和网络对应的读写异常IO操作
```
