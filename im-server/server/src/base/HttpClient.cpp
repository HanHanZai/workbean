...
CURLcode CHttpClient::Post(const string & strUrl, const string & strPost, string & strResponse)
{
    ...
    curl_easy_setopt(curl, CURLOPT_URL, strUrl.c_str()); /* 要操作的URL */
    curl_easy_setopt(curl, CURLOPT_FORBID_REUSE, 1); /* 防止后续连接重新使用它 */
    curl_easy_setopt(curl, CURLOPT_POST, 1); /* 发送Post命令 */
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, strPost.c_str()); /* 发送包含此数据的POST */
    curl_easy_setopt(curl, CURLOPT_READFUNCTION, NULL); /* 传递回调读入函数 */
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, OnWriteData); /* 传递回调写入函数 */
    curl_easy_setopt(curl, CURLOPT_WRITEDATA, (void *)&strResponse); /* 传递回调数据 */
    curl_easy_setopt(curl, CURLOPT_NOSIGNAL, 1); /* 不要安装信号处理程序 */
    curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT, 3); /* 连接阶段超时 */
    curl_easy_setopt(curl, CURLOPT_TIMEOUT, 3); /* 请求超时 */
