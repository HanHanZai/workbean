...
private:
	uchar_t*	m_buffer; /* 原数据对象 */
	uint32_t	m_alloc_size; /* 分配的内存大小 */
	uint32_t	m_write_offset; /* 当前数据偏移量 */
};
...
char* idtourl(uint32_t id);/* id转url，使用36位数进行计算 */
uint32_t urltoid(const char* url); /*url转id*/
