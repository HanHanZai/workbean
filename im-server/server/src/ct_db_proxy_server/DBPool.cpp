...
CResultSet::CResultSet(MYSQL_RES* res)
{
	...
	int num_fields = mysql_num_fields(m_res); /* 获取返回值的域数量 */
...
{
	/* 使用mysql_ping来自动重新连接数据库 */
	mysql_ping(mysql);

	m_stmt = mysql_stmt_init(mysql); /* 初始化mysql句柄对象 */
	if (!m_stmt) {
...
int CDBConn::Init()
{
	...
	mysql_options(m_mysql, MYSQL_OPT_RECONNECT, &reconnect); /* 自动连接选项 */
	mysql_options(m_mysql, MYSQL_SET_CHARSET_NAME, "utf8mb4"); /* 设置字符集名称 */
...
int CDBManager::Init()
{
	...
	char host[64]; /* 主机ip */
	char port[64]; /* 数据库端口 */
	char dbname[64]; /* 数据库名称 */
	char username[64]; /* 用户名称 */
	char password[64]; /* 用户密码 */
    char maxconncnt[64]; /* 最大连接数 */
	CStrExplode instances_name(db_instances, ','); /* 根据，进行分割，里面包含了主从数据库 */
