...
bool CGroupModel::removeGroup(uint32_t nUserId, uint32_t nGroupId, list<uint32_t>& lsCurUserId)
{
    ...
    if(pDBConn)
    {
        string strSql = "select creator from on_IMGroup where id="+int2string(nGroupId);
        ...
        if(pResultSet)
        {
            ...
            {
                //设置群组不可用。
                strSql = "update on_IMGroup set status=0 where id="+int2string(nGroupId);
        ...
        if (bRet) {
            strSql = "select userId from on_IMGroupMember where groupId="+int2string(nGroupId);
            CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
            if(pResultSet)
            {
                while (pResultSet->Next()) {
   ...
   if (!mapGroupId.empty())
    {
       CDBManager* pDBManager = CDBManager::getInstance();
        ...
        if (pDBConn)
        {
            ...
            string strSql = "select * from on_IMGroup where id in (" + strClause  + ") order by updated desc";
            CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
            if(pResultSet)
            {
                while (pResultSet->Next()) {
...
bool CGroupModel::modifyGroupMember(uint32_t nUserId, uint32_t nGroupId, IM::BaseDefine::GroupModifyType nType, set<uint32_t>& setUserId, list<uint32_t>& lsCurUserId)
{
    ...
    if(hasModifyPermission(nUserId, nGroupId, nType))
    {
        ...
        if(bRet)
        {
            incGroupVersion(nGroupId); /* 更新指定群组的id */
...
bool CGroupModel::insertNewGroup(uint32_t nUserId, const string& strGroupName, const string& strGroupAvatar, uint32_t nGroupType, uint32_t nMemberCnt, uint32_t& nGroupId)
{
    ...
    if (pDBConn)
    {
        string strSql = "insert into on_IMGroup(`name`, `avatar`, `creator`, `type`,`userCnt`, `status`, `version`, `lastChated`, `updated`, `created`) "\
        "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
bool CGroupModel::insertNewMember(uint32_t nGroupId, set<uint32_t>& setUsers)
{
    ...
    if(nGroupId != INVALID_VALUE &&  nUserCnt > 0)
    {
        ...
        if (pDBConn)
        {
            ...
            string strSql = "select userId from on_IMGroupMember where groupId=" + int2string(nGroupId) + " and userId in (" + strClause + ")";
            ...
            if (pDBConn)
            {
                ...
                if (pCacheConn)
                {
                    ...
                    if (!setHasUser.empty())
                    {
                        ...
                        strSql = "update on_IMGroupMember set status=0, updated="+int2string(nCreated)+" where groupId=" + int2string(nGroupId) + " and userId in (" + strClause + ")";
                        pDBConn->ExecuteUpdate(strSql.c_str());
                    }
                    strSql = "insert into on_IMGroupMember(`groupId`, `userId`, `status`, `created`, `updated`) values\
                    (?,?,?,?,?)";
                    ...
                    if(nIncMemberCnt != 0)
                    {
                        strSql = "update on_IMGroup set userCnt=userCnt+" + int2string(nIncMemberCnt) + " where id="+int2string(nGroupId);
                        pDBConn->ExecuteUpdate(strSql.c_str());
                    }
...
void CGroupModel::getUserGroupIds(uint32_t nUserId, list<uint32_t>& lsGroupId, uint32_t nLimited)
{
    ...
    if(pDBConn)
    {
        string strSql ;
        if (nLimited != 0) {
            strSql = "select groupId from on_IMGroupMember where userId=" + int2string(nUserId) + " and status = 0 order by updated desc, id desc limit " + int2string(nLimited);
        }
        else
        {
            strSql = "select groupId from on_IMGroupMember where userId=" + int2string(nUserId) + " and status = 0 order by updated desc, id desc";
        }
...
void CGroupModel::getGroupVersion(list<uint32_t> &lsGroupId, list<IM::BaseDefine::GroupVersionInfo> &lsGroup, uint32_t nGroupType)
{
    if(!lsGroupId.empty())
    {
        ...
        if(pDBConn)
        {
            ...
            string strSql = "select id,version from on_IMGroup where id in (" +  strClause  + ")";
            if(0 != nGroupType)
            {
...
bool CGroupModel::isInGroup2(uint32_t nUserId, uint32_t nGroupId)
{
	 bool bRet = false;
	 CDBManager* pDBManager = CDBManager::getInstance();
	 CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_slave");
	 if(pDBConn)
	 {
	      ...
	      //---------------------------------------------------
	        ...
	        CacheConn* pCacheConn = pCacheManager->GetCacheConn("group_member");
	        string strKey = "group_member_"+int2string(nGroupId);
	        string strSql2 = "select userId from on_IMGroupMember where groupId="+int2string(nGroupId);
	        CResultSet* pResultSet2 = pDBConn->ExecuteQuery(strSql2.c_str());
	         if(pResultSet2)
	         {
	            while (pResultSet2->Next()) {
	                uint32_t nId = pResultSet2->GetInt("userId");
	            	if(nUserId==nId)
	            	{
	            		bRet=true;
	            	}
	                pCacheConn->hset(strKey, int2string(nId), int2string(nGroupId));
	             }
	             delete pResultSet2;
	         }
	         pDBManager->RelDBConn(pDBConn);
...
bool CGroupModel::isInGroup(uint32_t nUserId, uint32_t nGroupId)
{
    ...
    {
    	return isInGroup2(nUserId,nGroupId);
    }
    return bRet;
}
...
bool CGroupModel::hasModifyPermission(uint32_t nUserId, uint32_t nGroupId, IM::BaseDefine::GroupModifyType nType)
{
    ...
    if(pDBConn)
    {
        string strSql = "select creator, type from on_IMGroup where id="+ int2string(nGroupId);
        CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
        if(pResultSet)
        {
            while (pResultSet->Next())
...
bool CGroupModel::removeMember(uint32_t nGroupId, set<uint32_t> &setUser, list<uint32_t>& lsCurUserId)
{
    ...
    if(pDBConn)
    {
        ...
        if (pCacheConn)
        {
            ...
            string strSql = "update on_IMGroupMember set status=1 where  groupId =" + int2string(nGroupId) + " and userId in(" + strClause + ")";
            pDBConn->ExecuteUpdate(strSql.c_str());
...
void CGroupModel::updateGroupChat(uint32_t nGroupId)
{
    ...
    if(pDBConn)
    {
        ...
        string strSql = "update on_IMGroup set lastChated=" + int2string(nNow) + " where id=" + int2string(nGroupId);
        pDBConn->ExecuteUpdate(strSql.c_str());
        pDBManager->RelDBConn(pDBConn);
    }
    else
...
bool CGroupModel::isValidateGroupId2(uint32_t nGroupId)
{
    bool bRet = false;
    CDBManager* pDBManager = CDBManager::getInstance();
    CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_slave");
    if(pDBConn)
    {
        string strSql = "select id from on_IMGroup where id=" + int2string(nGroupId)+" and status=0";
        CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
...
bool CGroupModel::isValidateGroupId(uint32_t nGroupId)
{
    ...
    {
       return isValidateGroupId2(nGroupId);
    }
}
...
bool CGroupModel::incGroupVersion(uint32_t nGroupId)
{
    ...
    if(pDBConn)
    {
        string strSql = "update on_IMGroup set version=version+1 where id="+int2string(nGroupId);
...
void CGroupModel::clearGroupMember(uint32_t nGroupId)
{
    ...
    if(pDBConn)
    {
        string strSql = "delete from on_IMGroupMember where groupId="+int2string(nGroupId);
        pDBConn->ExecuteUpdate(strSql.c_str());
        pDBManager->RelDBConn(pDBConn);
    }
    else
