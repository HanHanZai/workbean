...
bool CAudioModel::readAudios(list<IM::BaseDefine::MsgInfo>& lsMsg)
{
    ...
    if (pDBConn)
    {
        for (auto it=lsMsg.begin(); it!=lsMsg.end(); )
        {
            ...
            if(IM::BaseDefine::MSG_TYPE_AUDIO ==  nType)
            {
                string strSql = "select * from on_IMAudio where id=" + it->msg_data();
...
int CAudioModel::saveAudioInfo(uint32_t nFromId, uint32_t nToId, uint32_t nCreateTime, const char* pAudioData, uint32_t nAudioLen)
{
	...
	uint32_t nRealLen = nAudioLen - 4; /* 带了4个字节的头长度 */
    int nAudioId = -1;
	...
	if (!strPath.empty())
    {
        ...
        if (pDBConn)
        {
            ...
            /* 这里不怕被SQL注入嘛 */
            string strSql = "insert into on_IMAudio(`fromId`, `toId`, `path`, `size`, `duration`, `created`) "\
            "values(?, ?, ?, ?, ?, ?)";
