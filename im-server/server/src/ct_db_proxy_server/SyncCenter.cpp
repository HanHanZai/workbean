...
//m_pLock(new CLock())
{
    m_pCondGroupChat = new CCondition(m_pLockGroupChat); /* 条件变量 */
}
...
void CSyncCenter::init()
{
    ...
    CacheConn* pCacheConn = pCacheManager->GetCacheConn("unread"); //未读信息缓存
    if (pCacheConn)
    {
	    ...
	    if(strTotalUpdate != "")
        {
            m_nLastUpdate = string2int(strTotalUpdate);  /* 最后更新时间点 */
        }
        else
        {
            updateTotalUpdate(time(NULL)); /* 更新当前时间为刷新时间 */
        }
        if(strLastUpdateGroup.empty())
        {
            m_nLastUpdateGroup = string2int(strLastUpdateGroup); /* 群组最后更新的时间戳 */
        }
        else
        {
...
{
    CacheManager* pCacheManager = CacheManager::getInstance();
    CacheConn* pCacheConn = pCacheManager->GetCacheConn("unread"); /* 获取未读取的消息 */
    if (pCacheConn) {
...
void* CSyncCenter::doSyncGroupChat(void* arg)
{
    ...
    do{
        ...
        if(pDBConn)
        {
            string strSql = "select id, lastChated from on_IMGroup where status=0 and lastChated >=" + int2string(m_pInstance->getLastUpdateGroup());
    ...
    } while (m_pInstance->m_bSyncGroupChatWaitting && !(m_pInstance->m_pCondGroupChat->waitTime(5*1000))); /* 等待5s */
