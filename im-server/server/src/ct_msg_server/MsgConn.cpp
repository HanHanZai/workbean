...
void CMsgConn::SendUserStatusUpdate(uint32_t user_status)
{
    ...
    if (user_status == ::IM::BaseDefine::USER_STATUS_ONLINE) {
        ...
        pDbConn->SendPdu(&pdu2); //发送到数据在线状态
    }
}

void CMsgConn::updateUserStatus(uint32_t user_status)
{
    if (!m_bOpen) {
		return;
	}
...
void CMsgConn::Close(bool kick_user)
{
    ...
    //关闭时，将当前server人数上报到redis中
    uint32_t cur_conn_cnt = 0;
    cur_conn_cnt = CImUserManager::GetInstance()->GetUserConnCnt2();
    CDBServConn* pDbConn = get_db_serv_conn();
    IM::Server::IMMsgServInfo msg5;
    msg5.set_ip1("");
    msg5.set_ip2("");
    msg5.set_port(0);
    msg5.set_max_conn_cnt(0);
    msg5.set_cur_conn_cnt(cur_conn_cnt);
    msg5.set_host_name("");
    CImPdu pdu5;
    pdu5.SetPBMsg(&msg5);
    pdu5.SetServiceId(SID_OTHER);
    pdu5.SetCommandId(CID_OTHER_USER_CNT_UPDATE);
    if (pDbConn) {
        pDbConn->SendPdu(&pdu5);
    }
...
{
    updateUserStatus(::IM::BaseDefine::USER_STATUS_ONLINE); //心跳保持时，更新在线状态
    //响应
    SendPdu(pPdu);
}
