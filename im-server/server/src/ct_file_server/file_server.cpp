...
#include "ct_file_server/config_util.h"
#include "ct_file_server/file_client_conn.h"
#include "ct_file_server/file_msg_server_conn.h"
...
int main(int argc, char* argv[])
{
	...
	CConfigFileReader config_file("ct_fileserver.conf");

    /* ip对象1 */
    char* str_client_listen_ip1 = config_file.GetConfigName("Address");
    /* 监听ip */
    ...
    CStrExplode client_listen_ip_list2(str_client_listen_ip1, ';');
    ...
    for (uint32_t i = 0; i < client_listen_ip_list.GetItemCnt(); i++) {
        ConfigUtil::GetInstance()->AddAddress(client_listen_ip_list2.GetItem(i), client_listen_port);
    }
