    ...
    do {
        ...
        if (state_ == kTransferTaskStateReady) {
            ...
            if (file_role == CLIENT_REALTIME_SENDER) {
                state_ = kTransferTaskStateWaitingReceiver; /* 发送者已经就绪，等待接收者状态 */
            } else {
                state_ = kTransferTaskStateWaitingSender; /* 接收者已经就绪，等待发送者状态 */
            }
        } else {
...
/* offset一定等于当前已发送index*SEGMENT_SIZE */
