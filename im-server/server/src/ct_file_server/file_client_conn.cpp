...
#include "ct_file_server/transfer_task_manager.h"
...
void FileClientConn::OnConnect(net_handle_t handle) {
    ...
    g_file_client_conn_map.insert(make_pair(handle, this)); /* 唯一句柄号,绑定唯一FileClientConn对象 */
    /* 绑定回调 */
    netlib_option(handle, NETLIB_OPT_SET_CALLBACK, (void*)imconn_callback);
    /* 绑定回调数据 */
    netlib_option(handle, NETLIB_OPT_SET_CALLBACK_DATA, (void*)&g_file_client_conn_map);
    
    uint32_t socket_buf_size = NETLIB_MAX_SOCKET_BUF_SIZE; //128Kb
